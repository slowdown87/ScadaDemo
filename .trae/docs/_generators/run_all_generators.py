...
文档依赖关系:
    system_config.yaml (核心配置)
        ...
        ├──→ 茶饮料_HMI画面规格_auto.md
        └──→ 茶饮料_生产线工艺配置_auto.md
    hmi_screens_config.yaml (HMI配置)
        └──→ 茶饮料_HMI画面规格_auto.md
    FB_Spec_Template.yaml (功能块配置)
...
修改流程:
    ...
    2. 运行 python _generators/run_all_generators.py
    3. 检查生成的 *_auto.md 文档
"""
...
GENERATORS = {
    ...
    "HMI_Spec": {
        "script": "HMI_Screens_generator.py",
        "desc": "HMI画面规格说明书",
        "output": "01_Spec/茶饮料_HMI画面规格_auto.md",
        "input": "01_Spec/configs/hmi_screens_config.yaml"
    },
    "FB_Spec": {
        "script": "fb_spec_generator.py",
        "desc": "PLC功能块规格说明书",
        "output": "02_Arch/茶饮料_PLC功能块规格_auto.md",
        "input": "01_Spec/configs/FB_Spec_Template.yaml"
    },
    "Process_Flow": {
...
def main():
    ...
    if args.list:
        ...
        print("=" * 70)
        print(f"{'生成器':<15} {'说明':<20} {'输入':<35}")
        print("-" * 70)
        for key, gen in GENERATORS.items():
            print(f"{key:<15} {gen['desc']:<20} {gen['input']:<35}")
        print("=" * 70)
        return
