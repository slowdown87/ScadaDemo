...
CONFIG_CIP = SCRIPT_DIR.parent / "01_Spec" / "configs" / "process_recipe_templates.yaml"
OUTPUT_CIP = SCRIPT_DIR.parent / "05_Product" / "auto" / "茶饮料_CIP配方表_auto.md"
...
3. 检查生成的 `05_Product/auto/茶饮料_CIP配方表_auto.md`
...
def generate_cip_recipe():
    ...
    process_recipes = data.get('process_recipes', {})
    cip_recipes = process_recipes.get('programs', {})
    ...
    for idx, (key, cip) in enumerate(cip_recipes.items(), start=2):
        output.append("---")
        output.append(f"\n## {idx}. {cip.get('name', '')}\n")
    ...
    output.append("---")
    matrix_section = 2 + len(cip_recipes)
    output.append(f"\n## {matrix_section}. 产品切换清洗矩阵\n")

    switch_matrix = data.get('process_recipes', {}).get('product_switch_matrix', [])
