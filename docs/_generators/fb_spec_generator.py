...
def generate_document(config):
    ...
    lines.append("> 数据来源: 01_Spec/configs/fb_spec_config.yaml (自动生成)")
    lines.append("")
    ...
    lines.append("| 项目 | 内容 |")
    lines.append("| ---- | ---- |")
    lines.append("| **数据来源** | `01_Spec/configs/fb_spec_config.yaml` |")
    lines.append("| **生成器脚本** | `_generators/fb_spec_generator.py` |")
    lines.append("| **重新生成命令** | `python _generators/run_all_generators.py --gen=FB_Spec` |")
    lines.append("")
    lines.append("### 修改流程")
    lines.append("")
    lines.append("1. 编辑 `01_Spec/configs/fb_spec_config.yaml`")
    lines.append("2. 运行 `python _generators/run_all_generators.py --gen=FB_Spec`")
    lines.append("3. 检查生成的 `02_Arch/auto/茶饮料_PLC功能块规格_auto.md`")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## 1. 概述")
