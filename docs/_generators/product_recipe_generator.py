...
CONFIG_PRODUCT = SCRIPT_DIR.parent / "01_Spec" / "configs" / "product_recipe_templates.yaml"
OUTPUT_PRODUCT = SCRIPT_DIR.parent / "05_Product" / "auto" / "茶饮料_产品配方表_auto.md"
...
def generate_product_recipe():
    ...
    for idx, (key, product) in enumerate(products.items(), start=2):
        output.append("---")
        output.append(f"\n## {idx}. {product.get('name', '')} ({product.get('code', '')}) 配方\n")
        output.append(f"**描述**: {product.get('description', '')}\n")

        tea_spec = product.get('tea_spec', {})
        output.append(f"### {idx}.1 茶叶原料规格\n")
        output.append("| 项目 | 规格 |")
        output.append("|------|------|")
    ...
    output.append("---")
    acceptance_section = len(products) + 2
    output.append(f"\n## {acceptance_section}. 验收标准\n")

    output.append(f"### {acceptance_section}.1 感官指标\n")
    ...
    output.append(f"### {acceptance_section}.2 理化指标\n")
    ...
    output.append(f"### {len(products) + 1}.3 化学指标\n")
    output.append("| 项目 | 标准 | 单位 | 检验方法 |")
    output.append("|------|------|------|----------|")
    ...
    output.append(f"### {len(products) + 1}.4 微生物指标\n")
    output.append("| 项目 | 标准 | 单位 | 检验方法 |")
    output.append("|------|------|------|----------|")
