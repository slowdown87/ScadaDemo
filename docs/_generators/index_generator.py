...
CONFIG_FILE = DOCS_DIR / "01_Spec" / "configs" / "index_config.yaml"
OUTPUT_FILE = DOCS_DIR / "00_Index" / "auto" / "文档索引清单_auto.md"
...
def scan_directory_structure(base_path, config):
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        for file in files:
            if file.startswith('.'):
                continue
            if not (file.endswith('.md') or file.endswith('.yaml') or file.endswith('.yml') or file.endswith('.xlsx') or file.endswith('.py')):
                continue
...
def generate_directory_tree(structure, config):
    ...
    generators_data = structure.get('_generators', {'root': []})
    if generators_data.get('root'):
        lines.append("│")
        lines.append("├── _generators/                        ← ★统一生成器目录")
        for i, f in enumerate(sorted(generators_data['root'], key=lambda x: x['name'])):
            is_last = (i == len(generators_data['root']) - 1)
            prefix = "└──" if is_last else "├──"
            lines.append(f"│   {prefix} {f['name']}")
